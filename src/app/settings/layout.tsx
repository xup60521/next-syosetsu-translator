...
function GoBackLink({ children }: { children: React.ReactNode }) {
    ...
    return <Link href={"/"} prefetch={true}>
        {children}
    </Link>
}
...
export default function SettingsLayout({ children }: { children: React.ReactNode }) {
    ...
    // Derive selectedPanel from current pathname
    const selectedPanel = React.useMemo(() => {
        const currentNav = data.nav.find(item => pathname.startsWith(item.to));
                                ...
                                <SidebarMenu className="gap-2 p-2">
                                    ...
                                    {data.nav.map((item) => (
                                        <SidebarMenuItem key={item.name}>
                                            <Link href={item.to} prefetch={true}>
                                                <SidebarMenuButton
                                                    className="cursor-pointer font-mono uppercase font-bold rounded-xl"
